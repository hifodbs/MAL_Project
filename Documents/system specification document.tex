...
Given historical weather data (temperature, irradiance), predict the energy produced by the plant for the next timestamp.  
...
A custom KPI is used: for each day, the mean squared error (MSE) is calculated for each timestamp, and the daily mean is taken:

\[
C_{kpi} = \frac{\sum_{i=0}^{T} MSE_i}{|T|}
\]
...
\begin{itemize}
  ...
  \item Missing AC/DC power values:  
    \begin{itemize}
      ...
      \item During the day, linear interpolation (with median used for better quality).  
    \end{itemize}  
  \item Daily and total yield are forward-filled.  
\end{itemize}
...
FR-03 & The model shall detect drift in plant power production. \\
...
2. Server-side service (Flask) for predictions and sensor data access via a DAO.
...
\subsection{Training}
\begin{itemize}
    \item The model is trained incrementally from the first data point.
