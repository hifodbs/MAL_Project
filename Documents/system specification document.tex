...
An optional extension to a Long Short-Term Memory (LSTM) model for a 24 hour window forecast is provided
as proof of concept.
...
The error is expressed as a percentage to facilitate interpretation and with the possibility for the end user to
...
This setup simulates real-time sensor data using synthetic sensors and assume precise data transmission
...
\begin{enumerate}
    ...
    \item Selection of relevant features.
...
\begin{itemize}
  ...
  \item Missing AC/DC power values:  
    \begin{itemize}
      ...
      \item During the day, linear interpolation.  
    \end{itemize}  
\end{itemize}
...
\begin{itemize}
  ...
  \item Predict the power production for the next day using the ILSTM.
\end{itemize}
...
\begin{itemize}
    \item Online rolling evaluation monitors the custom KPI (daily mean MAE).
...
\begin{itemize}
    \item ADWIN monitors prediction errors (concept drift).
...
\begin{enumerate}
    ...
    \item ADWIN monitors for drift and sends data to the dashboard.
...
\textbf{Mitigation Strategy:} Continuous KPI monitoring (daily mean MAE) with alert thresholds. &
Medium \\
